...
def run_case(trial_idx, i, j):
    ...
    try:
        krome_start.run(
            ...
            length=length,
            geometry=geometry,
            folder_name_save=f'build_test_parallelize/build_{i}_{j}',
...
tasks = [(trial_idx, i, j) 
         for trial_idx, (i,j) in enumerate([(i, j) for i in range(0,2) for j in range(0,2)])]

N_TRIALS = len(tasks)
max_workers = 5 #Number of cores used 
